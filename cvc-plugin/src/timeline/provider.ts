...
import { TimelineGetResponse, InteractionSummary, CommitWithThoughts } from '../lsp/protocol';
...
export class TimelineTreeProvider implements vscode.TreeDataProvider<TimelineTreeItem> {
    private readonly _onDidChangeTreeData = new vscode.EventEmitter<TimelineTreeItem | undefined | void>();
    ...
    constructor(outputChannel: vscode.OutputChannel, lspClient: CvcLanguageClient) {
    ...
    async refresh(): Promise<void> {
        if (this.isLoading) {
            this.outputChannel.appendLine('Timeline refresh already in progress, skipping');
            return;
        }
