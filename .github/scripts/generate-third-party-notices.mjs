...
const lines = ['# Third-Party Notices', '', '<!-- Generated by .github/scripts/generate-third-party-notices.mjs; DO NOT EDIT. -->', '', 'Imported evidence preserves substantive text. For deterministic output only, CRLF is normalized to LF, trailing spaces and tabs are removed from each line, and each evidence text has exactly one terminal newline.', '', `Coverage: ${packages.filter(p => p.ecosystem === 'Rust').length} Rust packages; ${packages.filter(p => p.ecosystem !== 'Rust').length} production npm packages; ${evidence.size} unique evidence texts.`, '', '## Package inventory', '', '| Ecosystem | Package | SPDX license expression | Source / repository |', '| --- | --- | --- | --- |', ...packages.map(p => `| ${safe(p.ecosystem)} | ${safe(`${p.name}@${p.version}`)} | ${safe(p.license)} | ${safe(p.source)} |`), '', '## License, copyright, and notice texts', ''];
