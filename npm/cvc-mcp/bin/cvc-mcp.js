...
async function main() {
  ...
  const binName = platform === 'win32' ? 'cvc-mcp.exe' : 'cvc-mcp';
  ...
  if (!fs.existsSync(binPath)) {
    console.error(`CVC MCP not found locally. Downloading from ${downloadUrl}...`);
    ...
    try {
      ...
      console.error(`Failed to install CVC MCP: ${e.message}`);
      process.exit(1);
    }
  }
